...
{
    VKB_DEBUG() << "PlatformInputContext::reset()";
...
{
    VKB_DEBUG() << "PlatformInputContext::commit()";
    if (m_declarativeContext)
...
{
    VKB_DEBUG() << "PlatformInputContext::update():" << queries;
    if (m_declarativeContext)
...
{
    VKB_DEBUG() << "PlatformInputContext::invokeAction():" << action << cursorPosition;
}
